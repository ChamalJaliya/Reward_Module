...
jQuery(document).ready(function ($) {
  ...
  $(document.body).on("click", ".play-quest-button", function (e) {
    ...
    if (
      typeof quest_ajax_object === "undefined" ||
      typeof quest_ajax_object.play_nonce === "undefined"
    ) {
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          // After a 3-second delay, reset the button
          setTimeout(function () {
            $button
              .text("Play Quest")
              .prop("disabled", false)
  ...
  function fetchAndUpdateNotificationCount() {
    if (
      !studentIdentifier ||
      typeof quest_ajax_object === "undefined" ||
      typeof quest_ajax_object.fetch_nonce === "undefined"
    )
      return;
      ...
      success: function (response) {
        ...
        if (
          ...
          response.data.notifications.forEach(function (item) {
            ...
            if (!item.is_read) {
              listHtml +=
                '<button class="mark-as-read-btn" data-index="' +
                item.index +
                '">Mark Read</button>';
            }
            listHtml += "</li>";
          ...
          listHtml += "</ul>";
          listHtml +=
            '<div class="notifications-footer"><button class="mark-all-read-btn">Mark all as read</button></div>';
          ...
          $dropdown.html(
            '<div class="no-notifications">Could not load notifications.</div>'
          );
          console.error(
            "Fetch Error:",
  ...
  // Binding to .notifications-dropdown since it is always present when notifications are loaded.
  $(".notifications-dropdown").on("click", ".mark-as-read-btn", function (e) {
    e.preventDefault();
    e.stopPropagation(); // Prevent dropdown from closing
  ...
  $(".notifications-dropdown").on("click", ".mark-all-read-btn", function (e) {
    e.preventDefault();
    e.stopPropagation();
  ...
  function markNotificationRead(index, $buttonElement, markAll = false) {
    ...
    // console.log(
    //   "markNotificationRead called. Index:",
    //   index,
    //   "Mark All:",
    //   markAll
    // );
    ...
    if (
      !studentIdentifier ||
      typeof quest_ajax_object === "undefined" ||
      typeof quest_ajax_object.mark_read_nonce === "undefined" ||
      typeof quest_ajax_object.ajax_url === "undefined"
    ) {
  ...
  $(document).on("click", function (e) {
    ...
    if (
      !$bellArea.is(e.target) &&
      $bellArea.has(e.target).length === 0 &&
